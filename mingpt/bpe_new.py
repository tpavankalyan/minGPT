...
class ReasoningEncoder:
    ...
    def encode_and_show_work(self, text):
        ...
        for token in tokens:
            token_bytes = token.encode('utf-8')
            token_translated = token  # No byte fallback here
            token_merged = [token]    # No BPE merges
            token_ix = [self.encoder[token]] if token in self.encoder else []
    ...
    def decode(self, idx_list):
        return ' '.join([self.decoder[i] for i in idx_list])


def get_encoder_reasoning():
    # your new token list
    tokens = ['<|P|>', '<|Q|>', '<|S|>', 'bubble', 'cycle', 'gnome', 'heap', 'insertion', 'quick', 'selection', 'shell', '<|T|>', '<|A|>', '<|E|>'] + list('0123456789')
    encoder = {tok: i for i, tok in enumerate(tokens)}
    decoder = {i: tok for tok, i in encoder.items()}
    pad_token_id = encoder['<|P|>']
    return ReasoningEncoder(encoder, decoder, pad_token_id)

class BPETokenizerReasoning:
    """Minimal tokenizer class for the custom limited vocabulary, with reasoning/debug support."""

    def __init__(self):
        self.encoder = get_encoder_reasoning()
        self.pad_token_id = self.encoder.pad_token_id

    def __call__(self, text, return_tensors='pt', show_work=False):
        assert return_tensors == 'pt', "Only PyTorch tensor output is supported."
        assert isinstance(text, str), "Input text must be a string."

        if show_work:
            result = self.encoder.encode_and_show_work(text)
            token_ids = result['bpe_idx']
        else:
            token_ids = self.encoder.encode_and_show_work(text)['bpe_idx']

        if len(token_ids) == 0:
            raise ValueError("Input text could not be tokenized into known tokens.")
    ...
    def decode(self, idx):
        if isinstance(idx, torch.Tensor):
            idx = idx.tolist()
        if isinstance(idx[0], list):  # handle batched input
            idx = idx[0]
...
if __name__ == '__main__':
    ...
    text = "<|Q|> 4 3 2 1 <|S|> <|G|> <|T|> 4 3 1 2 <|T|> 4 1 2 3 <|A|> 1 2 3 4 <|E|>"
    e = get_encoder_reasoning()
